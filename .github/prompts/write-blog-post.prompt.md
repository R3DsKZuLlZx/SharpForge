---
mode: agent
description: Draft a new SharpForge blog post from a topic brief, matching the site's existing voice and passing the content validation suite.
---

# Write a blog post

The full instructions for this skill live in
[`.agents/skills/write-blog-post/SKILL.md`](../../.agents/skills/write-blog-post/SKILL.md).

Read that file and follow it. It is shared by all agents so the behaviour is
identical regardless of which tool is driving.

The specification it refers to is
[`.agents/skills/write-blog-post/reference.md`](../../.agents/skills/write-blog-post/reference.md).

Ask the user for the topic brief (topic, **angle**, category, sources) if it was
not supplied, then follow the shared skill's process end to end — including
running the content validation suite until it passes.

Do not duplicate guidance into this file — update the shared skill instead.

